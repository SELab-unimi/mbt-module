...
public class MarkovFileOpenDialog extends JDialog implements ActionListener {
    ...
    private JPanel getPnlMP() {
        if (pnlMP == null) {
            ...
            pnlMP.setBorder(javax.swing.BorderFactory.createEmptyBorder(5, 5,
                    5, 5));
            pnlMP.add(getTxtMarkovProcessFile(), BorderLayout.CENTER);
            pnlMP.add(getBtnMarkovProcess(), BorderLayout.EAST);
            pnlMP.add(lblMarkovProcessFile, BorderLayout.NORTH);
        }
        return pnlMP;
    }
    ...
    private JPanel getPnlStateFile() {
        if (pnlStateFile == null) {
            ...
            pnlStateFile.add(getTxtStateFile(), BorderLayout.CENTER);
            pnlStateFile.setBorder(javax.swing.BorderFactory.createEmptyBorder(
                    5, 5, 5, 5));
            pnlStateFile.setLayout(borderLayout6);
            pnlStateFile.add(getTxtStateFile(), BorderLayout.CENTER);
            pnlStateFile.add(getBtnStateFile(), BorderLayout.EAST);
            pnlStateFile.add(lblStateFile, BorderLayout.NORTH);
        }
        return pnlStateFile;
    }
    ...
    private JPanel getPnlEventFile() {
        if (pnlEventFile == null) {
            ...
            pnlEventFile.add(getTxtEventFile(), BorderLayout.CENTER);
            pnlEventFile.setBorder(javax.swing.BorderFactory.createEmptyBorder(
                    5, 5, 5, 5));
            pnlEventFile.setLayout(borderLayout7);
            pnlEventFile.add(getTxtEventFile(), BorderLayout.CENTER);
            pnlEventFile.add(getBtnEventFile(), BorderLayout.EAST);
            pnlEventFile.add(lblEventFile, BorderLayout.NORTH);
        }
        return pnlEventFile;
    }
