...
public class EventHandler {
	...
	private String getActionFromPolicy() {
		...
		System.out.println("STATE = " + monitor.currentState.getName());
		
		CharAction action = decisionRule.getAction(new IntegerState(Integer.parseInt(monitor.currentState.getName().substring(1))));
		
		System.out.println("ACTION = " + action.actionLabel());
